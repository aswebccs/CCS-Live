...
import { Edit, Trash2, ToggleRight, X } from 'lucide-react';
...
const Exams = ({ onNext }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
    const res = await fetch(`http://localhost:5000/api/exam-management/exams/${id}`, { method: 'DELETE' });
      const result = await res.json();
      if (result.success) {
        loadExams();
        showAlertOnce(result.message);
  ...
  const handleToggle = async (id) => {
    try {
    const res = await fetch(`http://localhost:5000/api/exam-management/exams/${id}/toggle`, { method: 'PATCH' });
      const result = await res.json();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow">
          ...
          {loading ? (
            ...
            exams.map((exam) => (
              <div key={exam.id} className="grid grid-cols-7 gap-4 items-center p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors">
                <div>
                  <button
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { setEditingExam(exam); setShowModal(true); }} className="p-2 hover:bg-blue-50 rounded-lg text-blue-600" title="Edit">
                    <Edit className="w-5 h-5" />
                  </button>
                  <button onClick={() => handleToggle(exam.id)} className={`p-2 hover:bg-gray-100 rounded-lg ${exam.is_active ? 'text-green-600' : 'text-gray-400'}`} title="Toggle">
                    <ToggleRight className="w-5 h-5" />
                  </button>
                  <button onClick={() => handleDelete(exam.id)} className="p-2 hover:bg-red-50 rounded-lg text-red-600" title="Delete">
