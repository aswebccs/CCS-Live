...
const Subcategories = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/exam-management/subcategories/${id}`, { method: 'DELETE' });
      const result = await res.json();
  ...
  const handleToggle = async (id) => {
    try {
      const res = await fetch(`http://localhost:5000/api/exam-management/subcategories/${id}/toggle`, { method: 'PATCH' });
      const result = await res.json();
            ...
            subcategories.map((sub) => (
              <div key={sub.id} className="grid grid-cols-5 gap-4 items-center p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => { setEditingSubcategory(sub); setShowModal(true); }}
