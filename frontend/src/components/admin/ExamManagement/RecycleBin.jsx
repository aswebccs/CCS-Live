...
const RecycleBin = () => {
  ...
  const loadTrashItems = async (tabType) => {
    try {
      ...
      if (tabType === 'categories') {
        url = `${API_ENDPOINTS.CATEGORIES_TRASH_LIST}?page=${page}&limit=${rowsPerPage}`;
      } else if (tabType === 'subcategories') {
        url = `${API_ENDPOINTS.SUBCATEGORIES_TRASH_LIST}?page=${page}&limit=${rowsPerPage}`;
      } else if (tabType === 'exams') {
  ...
  const getRestoreUrl = (tab, id) => {
    if (tab === 'categories') return API_ENDPOINTS.CATEGORY_RESTORE(id);
    if (tab === 'subcategories') return API_ENDPOINTS.SUBCATEGORY_RESTORE(id);
  ...
  const getPermanentDeleteUrl = (tab, id) => {
    if (tab === 'categories') return API_ENDPOINTS.CATEGORY_PERMANENT_DELETE(id);
    if (tab === 'subcategories') return API_ENDPOINTS.SUBCATEGORY_PERMANENT_DELETE(id);
  ...
  const getTitleColumn = () => {
    if (activeTab === 'categories') return 'Category Name';
    if (activeTab === 'subcategories') return 'Subcategory Name';
  ...
  const getNameValue = (item) => {
    if (activeTab === 'categories') return item.name;
    if (activeTab === 'subcategories') return item.name;
    if (activeTab === 'exams') return item.title;
    if (activeTab === 'questions') return item.question_text;
  ...
  const getCategoryName = (item) => {
    if (activeTab === 'subcategories') return item.category_name || '-';
    if (activeTab === 'exams') return item.category_name || '-';
    if (activeTab === 'questions') return item.exam_title || '-';
    return '';
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow overflow-x-auto">
          <div className="min-w-[900px]">
            {/* Headers */}
            <div className="grid grid-cols-5 gap-4 items-center p-4 bg-gray-50 border-b border-gray-200">
              <div>
              ...
              <div>
                <span className="text-sm font-semibold text-gray-700 uppercase">
                  {activeTab === 'categories' ? 'Parent' : activeTab === 'questions' ? 'Exam' : 'Category'}
                </span>
              </div>
              <div>
            ...
            {loading ? (
              ...
              items.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-5 gap-4 items-center p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors"
                >
                  <div>
