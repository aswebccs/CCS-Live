...
async function generateCertificatePdf(req, res) {
  try {
    ...
    // Idempotent behavior: if certificate already exists for same student+exam, return it.
    if (student_id && exam_id) {
      const existing = await pool.query(
        `SELECT id, student_id, exam_id, certificate_number, file_url, cloudinary_public_id, data_json, issued_at, status, created_at
         FROM certificates
      ...
      if (existingCertificate) {
        dbRes = await pool.query(
          `UPDATE certificates
           ...
           WHERE id = $6
           RETURNING *`,
          [fileUrl, publicId, dataJson, issued_at, 'issued', existingCertificate.id]
        );
      } else {
        const insertQuery = `INSERT INTO certificates (student_id, exam_id, certificate_number, file_url, cloudinary_public_id, data_json, issued_at, status)
          VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`;
        const values = [student_id, exam_id, certificate_number, fileUrl, publicId, dataJson, issued_at, 'issued'];
...
async function getCertificatesByStudent(req, res) {
  try {
    ...
    const q = 'SELECT id, student_id, exam_id, certificate_number, file_url, cloudinary_public_id, data_json, issued_at, status, created_at FROM certificates WHERE student_id = $1 ORDER BY issued_at DESC';
...
async function verifyCertificate(req, res) {
  try {
    ...
    const q = 'SELECT id, student_id, exam_id, certificate_number, file_url, cloudinary_public_id, data_json, issued_at, status FROM certificates WHERE certificate_number = $1 LIMIT 1';
...
async function getCertificateRecordByNumber(certificateNumber) {
  const q = `
    SELECT id, student_id, exam_id, certificate_number, file_url, cloudinary_public_id, data_json, issued_at, status
    FROM certificates
